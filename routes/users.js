...
router.get("/", authenticateToken, async (req, res) => {
	try {
		...
		// Step 1: Get all users you've already sent friend requests to
		...
		// Step 2: Query users excluding yourself and already sent requests
		const query = {
			_id: { $nin: [req.user._id, ...sentIds] },
			isActive: true,
		};
